...
class Paddle:
    ...
    def move(self, enemy_frect, ball_frect, table_size, fout =None, fin=None, gamescore=0):

        if self.move_getter == 'fifo':
            
            strout = '%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d:%d:%d:'
...
def game_loop(screen, paddles, ball, table_size, clock_rate, turn_wait_rate, score_to_win, display, fout, fin):
    score = [0, 0]
    ball = Ball(table_size, ball.size, ball.paddle_bounce, ball.wall_bounce, ball.dust_error, ball.init_speed_mag)
    gamescore = 0
    loop = 0
    ...
    while 1:
        old_score = score[:]
        # Signal end of round
        v1 = paddles[0].move(paddles[1].frect, ball.frect, table_size, fout, fin, gamescore)
        v2 = paddles[1].move(paddles[0].frect, ball.frect, table_size, fout, fin, -1*gamescore)

        if max(score) < score_to_win: ball.move(paddles, table_size)
        ball, score, gamescore = check_point(score, ball, table_size)
        if max(score) == score_to_win: gamescore *= 2

        if v1*v2 == 0: break
        elif v1*v2 == -1: return None
...
def init_game():
    ...
    p1 = 'fifo'
    #p1 = chaser_ai.chaser
    #p1 = directions_from_input
    p2 = chaser_ai.chaser

    paddles[0].move_getter = p1
    paddles[1].move_getter = p2
    ...
    while 1:
        ...
        print '[pong_server] Waiting for Client response ...'
        fin = open(PONGFIFO_IN)
